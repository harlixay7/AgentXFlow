...
fn create_real_git_project() -> PathBuf {
    let repo_dir =
        std::env::temp_dir().join(format!("agentxflow_live_repo_{}", uuid::Uuid::new_v4()));
...
async fn test_manual_live_end_to_end_system() {
    ...
    println!(
        "2. Generated cryptographic 256-bit authentication token: [{}...{}]",
        &initial_token[0..8],
    ...
    println!(
        "4. Verified Health Check response: status = {}",
        health_json["status"]
    );
    ...
    println!(
        "6. Verified MCP 'initialize': Protocol Version = {}",
        init_data["result"]["protocolVersion"]
    );
    ...
    println!(
        "7. Verified MCP 'tools/list': Discovered {} tools with parameter schemas",
        tools_array.len()
    );

    // 8. Register 2 Independent Autonomous Agents (Antigravity & Claude Code)
    let agent_a_resp = client
        .post(format!("{}/mcp", base_url))
        .header("Authorization", format!("Bearer {}", initial_token))
        .json(&json!({
            "jsonrpc": "2.0",
    ...
    let session_token_a = agent_a_json["result"]["session_token"]
        .as_str()
        .unwrap()
        .to_string();

    let agent_b_resp = client
        .post(format!("{}/mcp", base_url))
        .header("Authorization", format!("Bearer {}", initial_token))
        .json(&json!({
            "jsonrpc": "2.0",
    ...
    let session_token_b = agent_b_json["result"]["session_token"]
        .as_str()
        .unwrap()
        .to_string();

    println!(
        "8. Registered Agent A ({}) and Agent B ({})",
        &agent_a_id[0..8],
        &agent_b_id[0..8]
    );
    ...
    let task = coordinator
        .create_task(
            ...
            vec![
                (
                    "Create Session Struct".into(),
                    "Add session tracking to src/auth/session.rs".into(),
                    true,
                ),
                (
                    "Run Auth Unit Tests".into(),
                    "Execute cargo test to verify logic".into(),
                    true,
                ),
            ],
    ...
    println!(
        "11. Created Task #{} with 2 required steps and 1 acceptance criterion",
        task.id
    );
    ...
    let worktree_str = claim_data["result"]["worktree_path"]
        .as_str()
        .expect("Expected worktree path");
    let worktree_path = PathBuf::from(worktree_str);
    println!(
        "12. Task claimed by Agent A! Isolated Git worktree created at: {:?}",
        worktree_path
    );
    assert!(
    ...
    let task_b = coordinator
        .create_task(&proj.id, "Conflicting Task", "Desc", "HIGH", vec![], vec![])
        .unwrap();
    ...
    assert!(
        conflict_json.get("error").is_some(),
        "Conflicting scope must return error"
    );
    ...
    Command::new("git")
        ...
        .unwrap();
    Command::new("git")
        .args(&["commit", "-m", "feat: implement session manager"])
        .current_dir(&worktree_path)
        .output()
        .unwrap();
    ...
    for crit in &details_before.criteria {
        coordinator
            .satisfy_acceptance_criterion(&task.id, &crit.id, Some("Manual verification sign-off"))
            .unwrap();
    }
    // Execute real coordinator verification check in the worktree against HEAD
    let task_head = coordinator
        .git
        .get_worktree_head_sha(&worktree_path)
        .unwrap();
    coordinator
        .verify
        .execute_check(
    ...
    assert_eq!(
        submit_json["result"]["is_valid"], true,
        "Submission rejected: {:?}",
        submit_json["result"]["rejection_reasons"]
    );
    println!(
        "17. Task verified and approved by Coordinator Verification Gate! (State moved to REVIEW)"
    );
    ...
    assert!(
        final_details.proof_bundle.is_some(),
        "Proof bundle must be generated and sealed"
    );
    let bundle = final_details.proof_bundle.unwrap();
    println!(
        "18. Inspected TaskDetails & Proof Bundle: SHA-256 Digest = [{}...{}]",
        &bundle.proof_hash[0..8],
        &bundle.proof_hash[bundle.proof_hash.len() - 8..]
    );

    // 19. Enqueue Task into Serialized Merge Queue
    let q_item = coordinator
        .merge
        .enqueue_task(
            &proj.id,
            &task.id,
    ...
    println!(
        "19. Enqueued Task into Serialized Merge Queue at position #{}",
        q_item.position
    );

    // 20. Process Merge Queue in Isolated Integration Worktree
    let merge_attempt = coordinator
        .merge
        .process_merge(&proj.id, &repo_dir, &q_item)
        .expect("Merge failed");
    ...
    println!(
        "21. Target branch ref 'main' successfully advanced to: {}",
        &main_head[0..12]
    );

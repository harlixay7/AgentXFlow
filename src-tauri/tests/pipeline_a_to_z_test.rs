...
async fn test_entire_pipeline_from_a_to_z() {
    ...
    mcp_server
        .start()
        .await
        .expect("Failed to start live MCP server");
    ...
    let session_token_a = reg_a_json["result"]["session_token"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let session_token_b = reg_b_json["result"]["session_token"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    println!(
        "          Isolated AppData Worktree Created at: {:?}",
        worktree_a
    );
    ...
    assert!(hoard_json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("Anti-hoarding"));
    ...
    assert!(conflict_json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("Scope collision"));
    ...
    Command::new("git")
        ...
        .unwrap();
    Command::new("git")
        .args(&["commit", "-m", "feat: add auth token validation"])
        .current_dir(&worktree_a)
        .output()
        .unwrap();
    ...
    for crit in &details_a.criteria {
        coordinator
            .satisfy_acceptance_criterion(&task_a_id, &crit.id, Some("Signed off by Lead Reviewer"))
            .unwrap();
    }
    ...
    println!(
        " [Step 19] Cryptographic Proof Bundle Generated: SHA-256 = [{}...{}]",
        &proof_a.proof_hash[..8],
        &proof_a.proof_hash[proof_a.proof_hash.len() - 8..]
    );
    ...
    Command::new("git")
        ...
        .unwrap();
    Command::new("git")
        .args(&["commit", "-m", "feat: add database storage layer"])
        .current_dir(&worktree_b)
        .output()
        .unwrap();
    ...
    for step in &details_b.steps {
        coordinator
            .complete_step(&step.id, None, Some("DB tests passed"))
            .unwrap();
    }
    for crit in &details_b.criteria {
        coordinator
            .satisfy_acceptance_criterion(&task_b_id, &crit.id, Some("Reviewer sign-off"))
            .unwrap();
    }
    ...
    // ------------------------------------------------------------------------
    // Enqueue Task A
    let q_item_a = coordinator
        .enqueue_task_by_id(&proj.id, &task_a_id)
    ...
    // Enqueue Task B
    let q_item_b = coordinator
        .enqueue_task_by_id(&proj.id, &task_b_id)
    ...
    let out_of_order = coordinator
        .merge
        .process_merge_by_id(&q_item_b.id, &repo_dir);
    ...
    let merge_a = coordinator
        .merge
        .process_merge_by_id(&q_item_a.id, &repo_dir)
        .expect("Merge A failed");
    ...
    pool.lock()
        .execute(
            "UPDATE tasks SET base_sha = ?1, head_sha = ?2, state = 'REVIEW' WHERE id = ?3",
            rusqlite::params![target_sha_1, head_b_new, task_b_id],
        )
        .unwrap();
    coordinator
        .verify
    ...
    let q_item_b_rebased = coordinator
        .enqueue_task_by_id(&proj.id, &task_b_id)
        .expect("Re-enqueue B failed");
    let merge_b = coordinator
        .merge
        .process_merge_by_id(&q_item_b_rebased.id, &repo_dir)
        .expect("Merge B failed");
    ...
    println!(
        " [Step 26] Task B Rebased & Successfully Merged into 'main' (CAS SHA: {}) -> State: DONE",
        &target_sha_2[..8]
    );
    ...
    let sim_task = coordinator
        .create_task(
            &proj.id,
            "Simulated Interrupted Task",
            "Crash test",
            "LOW",
            vec![],
            vec![],
    ...
    pool.lock()
        .execute(
            "UPDATE tasks SET state = 'CLAIMING', substate = 'CLAIMING' WHERE id = ?1",
            [&sim_task.id],
        )
        .unwrap();

...
async fn test_multi_agent_concurrent_collaboration_and_merge() {
    ...
    println!(
        "   ✔ Registered Agent 1: {} [{}]",
        agent_antigravity.name, agent_antigravity.id
    );
    println!(
        "   ✔ Registered Agent 2: {} [{}]",
        agent_claude.name, agent_claude.id
    );
    println!(
        "   ✔ Registered Agent 3: {} [{}]",
        agent_cursor.name, agent_cursor.id
    );
    ...
    let t3 = coordinator
        .create_task(
            ...
            vec![
                ...
                (
                    "Run UI Tests".into(),
                    "Component rendering tests".into(),
                    true,
                ),
            ],
    ...
    let claimed1 = coordinator
        .claim_task(&t1.id, &agent_antigravity.id)
        .expect("Antigravity claim failed");
    let claimed2 = coordinator
        .claim_task(&t2.id, &agent_claude.id)
        .expect("Claude claim failed");
    let claimed3 = coordinator
        .claim_task(&t3.id, &agent_cursor.id)
        .expect("Cursor claim failed");

    assert_eq!(
        claimed1.assigned_agent_id,
        Some(agent_antigravity.id.clone())
    );
    ...
    println!(
        "   ✔ Task 1 claimed by Antigravity (Branch: {:?})",
        claimed1.branch_name
    );
    println!(
        "   ✔ Task 2 claimed by Claude Code (Branch: {:?})",
        claimed2.branch_name
    );
    println!(
        "   ✔ Task 3 claimed by Cursor IDE (Branch: {:?})",
        claimed3.branch_name
    );
    ...
    let collision_risk = coordinator
        .scope
        .calculate_collision_risk(&t1.id, &t2.id)
        .unwrap();
    println!(
        "   ✔ Calculated Collision Risk between Task 1 & Task 2: Score = {} (Clean: 0.0)",
    ...
    let get_task_step_ids = |task_id: &str| -> Vec<String> {
        let conn = pool.lock();
        let mut stmt = conn
            .prepare("SELECT id FROM task_steps WHERE task_id = ?1 ORDER BY order_index ASC")
            .unwrap();
        let rows = stmt
    ...
    for tid in [&t1.id, &t2.id, &t3.id] {
        ...
        for step_id in step_ids {
            let s = coordinator
                .complete_step(
                    &step_id,
                    None,
                    Some(r#"{"stdout": "check ok", "exit_code": 0}"#),
                )
                .unwrap();
        ...
        for crit in &details.criteria {
            coordinator
                .satisfy_acceptance_criterion(tid, &crit.id, Some("Automated test pass"))
                .unwrap();
        }
    ...
    let verify1 = coordinator
        .submit_task(&t1.id, &agent_antigravity.id)
        .expect("Verification 1 failed");
    let verify2 = coordinator
        .submit_task(&t2.id, &agent_claude.id)
        .expect("Verification 2 failed");
    let verify3 = coordinator
        .submit_task(&t3.id, &agent_cursor.id)
        .expect("Verification 3 failed");

    assert!(
        verify1.is_valid,
        "Task 1 verification failed: {:?}",
        verify1.rejection_reasons
    );
    assert!(
        verify2.is_valid,
        "Task 2 verification failed: {:?}",
        verify2.rejection_reasons
    );
    assert!(
        verify3.is_valid,
        "Task 3 verification failed: {:?}",
        verify3.rejection_reasons
    );

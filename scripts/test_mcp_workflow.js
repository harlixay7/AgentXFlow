...
async function runE2EWorkflow() {
  ...
  try {
    ...
    console.log('[OK] Health Status:', health.status, `(Protocol: ${health.protocol_version})`);
    ...
    console.log('[OK] Context Pack Contract Hash:', ctx.contract_hash);
    console.log('[OK] Project Rules:', ctx.project_rules);
    ...
    console.log(`[OK] Acquired ${leases.length} exclusive scope locks:`, leases.map((l) => l.pattern));
    ...
    console.log(`[OK] Current Merge Queue Size: ${queue.length}`);

    // 10. Release scope
    console.log(`\n[Step 10] Releasing Scope Locks (scope.release)...`);
    const rel = await sendRpc('scope.release', { task_id: targetTask.id });
    console.log('[OK] Scope Release Result:', rel);

    console.log('\n====================================================');
    console.log('All 10 MCP protocol tools called and verified successfully.');
    console.log('====================================================');
  } catch (err) {
    console.error('[ERROR] E2E Workflow Error:', err.message);
  }
}
